...
@
<<label=token_count, include=FALSE, echo=FALSE>>=
...
\vspace{-5pt}
<<label=soc.tok, include=FALSE, echo=FALSE>>=
...
\begin{figure}[ht]
  \vspace{-5pt}
  \begin{center}
<<fig=TRUE, echo=FALSE, height=4, width=10>>=
<<soc.tok>>
@
  \end{center}
  \vspace{-20pt}
...
<<label=soc.speaker, include=FALSE, echo=FALSE>>=
library(plyr)
...
<<label=age_by_speaker, include=FALSE, echo=FALSE>>=
...
<<label=phono, echo=FALSE, include=FALSE>>=
par(mfrow=c(1,3))
...
<<label=sib.intensity, include=FALSE, echo=FALSE>>=
...
@
  ...
  \end{center}
\end{figure}
<<label=prec.intensity, include=FALSE, echo=FALSE>>=
par(mfrow=c(1, 3), cex=.75, cex.main=1)
...
<<label=prec.pitch, include=FALSE, echo=FALSE>>=
par(mfrow=c(1, 3), cex=.75, cex.main=1)
hist(dat$prec_segment_pitch05, main="Prec. Segment Pitch 5%",
     xlab="Pitch", col="darkgrey", freq=FALSE)
lines(density(dat$prec_segment_pitch05, na.rm=T), col="red", lwd=2)
hist(dat$prec_segment_pitch25, main="Prec. Segment Pitch 25%",
     xlab="Pitch", col="darkgrey", freq=FALSE)
lines(density(dat$prec_segment_pitch25, na.rm=T), col="red", lwd=2)
hist(dat$prec_segment_pitch50, main="Prec. Segment Pitch 50%",
     xlab="Pitch", col="darkgrey", freq=FALSE)
lines(density(dat$prec_segment_pitch50, na.rm=T), col="red", lwd=2)
@
\begin{figure}[h!]
  \begin{center}
<<fig=TRUE, echo=FALSE, height=3>>=
...
<<label=random.effect.groups.0, echo=FALSE, eval=FALSE, include=FALSE>>=
library(lme4)
(n.unique.words <- length(table(dat$word)))
(n.hapax <- length(table(dat$word)[table(dat$word) == 1]))
@
<<label=random.effect.groups, echo=FALSE, eval=FALSE, include=FALSE>>=
...
abline(a=6.9, b=-1.5,
            col="red", lwd=2)
@
<<label=random.effect.groups.2, echo=FALSE, eval=FALSE, include=FALSE>>=
...
<<label=sib_dur_hist, echo=FALSE, eval=FALSE>>=
...
\begin{figure}[h!]
  \begin{center}
    \begin{minipage}[t]{0.5\linewidth}
      \begin{center}
<<fig=TRUE, echo=FALSE, height=5.5>>=
<<sib_dur_hist>>
@
      \end{center}
    \end{minipage}
...
<<label=sib_dur_qq, include=FALSE, echo=FALSE>>=
...
\begin{figure}[h!]
  \begin{center}
    \begin{minipage}[t]{\linewidth}
      \begin{center}
<<fig=TRUE, echo=FALSE, height=2.25>>=
<<sib_dur_qq>>
@
      \end{center}
    \end{minipage}
...
<<label=prec_dur_hist, echo=FALSE, eval=FALSE>>=
par(cex=1, cex.lab=2, cex.main=2, cex.axis=2, mai=c(1, 1, 0, .5))
...
<<label=prec_dur_qq, include=FALSE, echo=FALSE>>=
par(mfrow=c(1, 3), cex=.5, cex.main=1.5, cex.lab= 1.5, cex.axis=1,
    mai=c(.5, .65, .5, 0))
...
<<label=cog_hist, echo=false, include=false>>=
...
\begin{figure}[h!]
  \begin{center}
    \begin{minipage}[t]{.9\linewidth}
      \begin{center}
<<fig=TRUE, echo=FALSE, height=6.5>>=
<<cog_hist>>
@
      \end{center}
    \end{minipage}
...
<<label=cog_qq, echo=FALSE, include=TRUE>>=
...
<<label=vls_hist, echo=FALSE, include=FALSE>>=
...
<<label=vls_qq, echo=FALSE, include=FALSE>>=
...
<<label=pretty.print, echo=FALSE, results=tex, strip.white=false>>=
...
<<label=sib.dur.0, echo=FALSE, results=tex, strip.white=false>>=
library(xtable)
...
<<label=sib.dur.1, echo=FALSE, results=tex>>=
...
@
<<label=sib.dur.3, echo=FALSE, results=tex>>=
...
<<label=sib.dur.3.1, echo=FALSE, results=tex>>=
...
<<label=sib.dur.4, echo=FALSE, results=tex>>=
...
@
<<label=sib.dur.4.2, echo=FALSE, results=tex>>=
...
<<label=sib.dur.4.4, echo=FALSE, results=tex>>=
print(xtable(anova(sib.dur.2.1, sib.dur.2.0)), table.position="h!")
@

Having \texttt{prec.int25} is no better than not having it in the model, so
it is safe to drop the term.\newline

<<label=sib.dur.4.3, echo=FALSE, results=tex>>=
...
<<label=sib.dur.5, echo=FALSE, results=tex>>=
...
<<label=sib.dur.5.1, echo=FALSE, results=tex>>=
...
<<label=sib.dur.res, echo=FALSE, include=FALSE>>=
...
<<label=sib.dur.6, echo=FALSE, results=tex>>=
...
\vspace{-10pt}
<<label=sib.dur.by.age, echo=FALSE, include=FALSE>>=
...
<<label=sib.dur.by.sib, echo=FALSE, include=FALSE>>=
...
<<label=sib.dur.int, echo=FALSE, include=FALSE>>=
...
source("data.R", echo=TRUE, print.eval=TRUE, max.deparse.length=500)
@
...
<<echo=TRUE, eval=FALSE, stip.white=false>>=
...
<<random.effect.groups.0>>
@
\begin{center}
  \begin{minipage}[t]{\linewidth}
    \begin{center}
<<echo=TRUE, eval=TRUE, fig=TRUE, height=3>>=
